...
class URLDiscItem(comparatorOutput: Int, sound: SoundEvent, props: Properties) :
  RecordItem(comparatorOutput, sound, props, 0) {
  companion object {
    const val CUSTOM_DISC_TRANSLATION_KEY = "item.${ModContent.MOD_ID}.custom_record.tooltip"
    ...
    fun createDiscItem(
      ...
      duration: Int,
      name: String,
      variant: DiscVariant = DiscVariant.RED,
      radius: Int = 30,
      locked: Boolean = false,
      owner: String = ""
    ): ItemStack {
      val item = ItemStack(ModItems.CUSTOM_RECORD.get())
  ...
  override fun use(world: Level, player: Player, pUsedHand: InteractionHand): InteractionResultHolder<ItemStack> {
    ...
    if (!world.isClientSide && stackInHand.`is`(ModItems.CUSTOM_RECORD.get())) {
      ...
      ModNetwork.CHANNEL.sendToClientPlayer(player as ServerPlayer, OpenDiscScreenS2CPacket(stackInHand))
      return InteractionResultHolder.success(player.getItemInHand(pUsedHand))
    }
