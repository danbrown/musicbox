...
class MusicDiscScreen(menu: MusicDiscMenu, private val inv: Inventory) :
  AbstractContainerScreen<MusicDiscMenu>(menu, inv, Component.literal("")) {
  companion object {
    const val LOCKED_DISC_TRANSLATION_KEY = "gui.${ModContent.MOD_ID}.custom_record.locked"
    const val DOWNLOADING_DISC_TRANSLATION_KEY = "gui.${ModContent.MOD_ID}.custom_record.downloading"
    const val DOWNLOADING_ERROR_DISC_TRANSLATION_KEY = "gui.${ModContent.MOD_ID}.custom_record.downloading_error"
    const val DOWNLOADING_SUCCESS_DISC_TRANSLATION_KEY = "gui.${ModContent.MOD_ID}.custom_record.downloading_success"
    const val NO_RECORD_TRANSLATION_KEY = "gui.${ModContent.MOD_ID}.custom_record.no_record"
    const val URL_INVALID_TRANSLATION_KEY = "gui.${ModContent.MOD_ID}.custom_record.url_invalid"
    const val YOUTUBE_INVALID_TRANSLATION_KEY = "gui.${ModContent.MOD_ID}.custom_record.youtube_invalid"
    const val URL_TOO_LONG_TRANSLATION_KEY = "gui.${ModContent.MOD_ID}.custom_record.url_too_long"
    const val RADIUS_TOO_SMALL_TRANSLATION_KEY = "gui.${ModContent.MOD_ID}.custom_record.radius_too_small"
    const val RADIUS_TOO_BIG_TRANSLATION_KEY = "gui.${ModContent.MOD_ID}.custom_record.radius_too_big"
    const val DISC_SAVED_TRANSLATION_KEY = "gui.${ModContent.MOD_ID}.custom_record.disc_saved"

    const val DURATION_TRANSLATION_KEY = "gui.${ModContent.MOD_ID}.custom_record.duration"
    const val SONG_NAME_TRANSLATION_KEY = "gui.${ModContent.MOD_ID}.custom_record.song_name"
    const val RADIUS_TRANSLATION_KEY = "gui.${ModContent.MOD_ID}.custom_record.radius"
    const val YOUTUBE_URL_TRANSLATION_KEY = "gui.${ModContent.MOD_ID}.custom_record.youtube_url"

    const val URL_FIELD_KEY = "gui.${ModContent.MOD_ID}.custom_record.url.field"
    const val DURATION_FIELD_KEY = "gui.${ModContent.MOD_ID}.custom_record.duration.field"
    const val SONG_NAME_FIELD_KEY = "gui.${ModContent.MOD_ID}.custom_record.song_name.field"
    const val RADIUS_FIELD_KEY = "gui.${ModContent.MOD_ID}.custom_record.radius.field"
  }

  private val TEXTURE = ResourceLocation(ModContent.MOD_ID, "textures/gui/record_input.png")
  ...
  private fun subInit() {
    ...
    this.DURATION_FIELD!!.value =
      if (playerDurationValue.isEmpty()) DEFAULT_DURATION.toString() else playerDurationValue
    ...
    this.RADIUS_FIELD!!.value =
      if (playerRadiusValue.isEmpty() || playerRadiusValue.toInt() <= 0) DEFAULT_RADIUS.toString() else playerRadiusValue
  ...
  override fun keyPressed(pKeyCode: Int, pScanCode: Int, pModifiers: Int): Boolean {
    if (pKeyCode == GLFW.GLFW_KEY_ESCAPE || pKeyCode == GLFW.GLFW_KEY_ENTER) {
      ...
      ModNetwork.CHANNEL.sendToServer(
        SaveDiscUrlC2SPacket(
          this.TEXT_FIELD!!.value,
          duration,
          this.SONG_NAME_FIELD!!.value,
          radius,
          pKeyCode == GLFW.GLFW_KEY_ENTER,
          if (menu.stack?.tag?.getFloat(URLDiscItem.PITCH_TAG_KEY) == 0.0f) 1.0f else menu.stack?.tag?.getFloat(
            URLDiscItem.PITCH_TAG_KEY
          )!!,
        )
      )
    ...
    val textField =
      (!this.TEXT_FIELD!!.keyPressed(pKeyCode, pScanCode, pModifiers) && !this.TEXT_FIELD!!.canConsumeInput())
    val durationField =
      (!this.DURATION_FIELD!!.keyPressed(pKeyCode, pScanCode, pModifiers) && !this.DURATION_FIELD!!.canConsumeInput())
    val songNameField =
      (!this.SONG_NAME_FIELD!!.keyPressed(pKeyCode, pScanCode, pModifiers) && !this.SONG_NAME_FIELD!!.canConsumeInput())
    val radiusField =
      (!this.RADIUS_FIELD!!.keyPressed(pKeyCode, pScanCode, pModifiers) && !this.RADIUS_FIELD!!.canConsumeInput())
    return if (textField || durationField || songNameField || radiusField) true else super.keyPressed(
      pKeyCode,
      pScanCode,
      pModifiers
    )
  ...
  override fun render(guiGraphics: GuiGraphics, mouseX: Int, mouseY: Int, delta: Float) {
    ...
    guiGraphics.drawString(
      font,
      Component.translatable(DURATION_FIELD_KEY),
      TEXT_FIELD!!.x - 54,
      DURATION_FIELD!!.y,
      0x555555
    )
    guiGraphics.drawString(
      font,
      Component.translatable(SONG_NAME_FIELD_KEY),
      TEXT_FIELD!!.x - 54,
      SONG_NAME_FIELD!!.y,
      0x555555
    )
    guiGraphics.drawString(
      font,
      Component.translatable(RADIUS_FIELD_KEY),
      TEXT_FIELD!!.x - 54,
      RADIUS_FIELD!!.y,
      0x555555
    )
